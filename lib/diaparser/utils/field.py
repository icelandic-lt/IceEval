...
class Field(RawField):
    ...
    def __init__(self, name, pad=None, unk=None, bos=None, eos=None,
                 lower=False, use_vocab=True, tokenize=None, fn=None, mask_token_id=0, tokenizer=None):
        self.name = name
        self.pad = pad
        self.unk = unk
        self.bos = bos
        ...
        if self.tokenizer:
            self.ids_to_tokens = {val: key for key, val in tokenizer.vocab.items()}
        else:
            self.ids_to_tokens = {}
    ...
    def preprocess(self, sequence):
        ...
        if self.tokenize is not None:
            ...
            sequence = [self.ids_to_tokens[i] for i in encoded]
        if self.lower:
