...
from rouge_score import tokenize as rouge_tokenize
...
def rouge_tokenize_utf(text, stemmer):
    """A Unicode-friendly (and non-stemming) version of rouge_score's tokenization function."""
    text = text.lower()

    # Before calculating the ROUGE score, rouge_score's tokenizer removes all non-alphanumeric ([^a-z0-9]) ASCII
    # characters from the text. This includes Unicode characters, so "fjölmiðlar" would become "fj lmi lar". We replace
    # it with [^\w]+, which preserves all alphabetic Unicode characters.
...
# Override rouge_score's tokenizer with our Unicode-friendly version.
rouge_tokenize.tokenize = rouge_tokenize_utf
...
def process_text(text, max_length=None):
    tokenized = [re.findall(r'[\w-]+', s) for s in text]
        ...
        if total_tokens > max_length:
            ...
            for sentence in tokenized:
                ...
                if length <= remaining:
                    ...
                    if remaining:
                        truncated.append(sentence[:remaining])
                    break

            tokenized = truncated

    return [" ".join(s) for s in tokenized]
...
class ExtractiveSummarizer(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.hparams.no_test_block_trigrams = True
        ...
        for idx, (source, source_ids, target) in enumerate(zip(sources, selected_ids, targets)):
            ...
            for sent_idx, i in enumerate(source_ids):
                ...
                if self.hparams.no_test_block_trigrams or (not block_trigrams(candidate, current_prediction)):
                    tokens = re.findall(r'[\w-]+', candidate)
                    current_length += len(tokens)
                    current_prediction.append((i, candidate))
            ...
            current_indexes = {idx for idx, s in current_prediction}
            current_prediction = [s for idx, s in sorted(current_prediction)]
            current_labels = [int(pos in current_indexes) for pos in range(len(source))]
            labels_out.append(current_labels)
            ...
            if self.hparams.test_use_pyrouge:
                ...
                prediction = "\n".join(process_text(current_prediction, 100))
                target = "\n".join(process_text(target.replace("<q>", "\n").splitlines(), 100))
        ...
        result_path = os.path.join(self.hparams.default_root_dir, 'results.jsonl')
        with open(result_path, 'a', encoding='utf-8') as f:
            for score in rouge_outputs:
                s = {
                    'r1-precision': score['rouge1'].precision,
                    'r1-recall': score['rouge1'].recall,
                    'r1-fmeasure': score['rouge1'].fmeasure,
                    'r2-precision': score['rouge2'].precision,
                    'r2-recall': score['rouge2'].recall,
                    'r2-fmeasure': score['rouge2'].fmeasure,
                    'rL-precision': score['rougeLsum'].precision,
                    'rL-recall': score['rougeLsum'].recall,
                    'rL-fmeasure': score['rougeLsum'].fmeasure,
                }
